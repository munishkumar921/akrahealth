...
/* ===== Canvas & BG ===== */
const CANVAS = { width: 630, height: 600 };
const BG_PATH = ref(null);     // source (string | File | Blob)
...
/* ===== Tools & State ===== */
// modes: select | draw | erase | rect | circle | text
...
/* Drawing data */
const lines = ref([]);     // freehand strokes (on draw layer)
const rects = ref([]);     // vector shapes
const circles = ref([]);
const texts = ref([]);
...
/* ---------- Load BG from form ---------- */
watch(
  () => props.form?.annotate_image,
  (val) => {
    BG_PATH.value = val || null;
  },
  { immediate: true }
);

/* Load/Reload the HTMLImage when BG_PATH changes */
watch(BG_PATH, (src) => {
  bgImage.value = null;
  if (!src) return;
  ...
  if (src instanceof File || src instanceof Blob) {
    ...
    // normalize relative Laravel paths (e.g., "storage/..." -> "/storage/...")
    const s = String(src);
    img.src = s.startsWith("/") ? s : `/${s}`;
    // If you see caching issues, use: img.src = `${img.src}?t=${Date.now()}`
  }
});
...
/* Update selected node live from toolbar controls */
...
const savePNG = async () => {
  ...
  form.append('image', blob, `annotation-${Date.now()}.png`);
  const token = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');

  const resp = await fetch('/doctor/updated-annotation', {
    method: 'POST',
...
<template>
  <div>
    ...
    <div style="display:flex; gap:8px; align-items:center; margin-bottom:10px; flex-wrap:wrap">
      <button type="button" @click="undo" title="Undo">↩️</button>
      <button type="button" @click="setMode('select')" :class="{ active: mode === 'select' }"
        title="Select">🖱️</button>
      <button type="button" @click="setMode('draw')" :class="{ active: mode === 'draw' }" title="Pen">✏️</button>
      <button type="button" @click="setMode('erase')" :class="{ active: mode === 'erase' }" title="Eraser">🩹</button>
      <button type="button" @click="setMode('text')" :class="{ active: mode === 'text' }" title="Text">T</button>
      <button type="button" @click="setMode('rect')" :class="{ active: mode === 'rect' }" title="Rectangle">▭</button>
      <button type="button" @click="setMode('circle')" :class="{ active: mode === 'circle' }" title="Circle">◯</button>
      <button type="button" :id="id" @click="savePNG" style="display: none;">💾 Save</button>
    </div>

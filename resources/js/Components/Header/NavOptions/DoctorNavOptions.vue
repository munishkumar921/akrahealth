...
<template>
    <div v-if="visible" class="d-flex gap-4 align-items-center ml-2 doctor-nav">
        <!-- PATIENT SUMMARY -->
        <div v-if="$page.props.selected_patient && isDoctorMode" class="nav-card shadow-lg position-relative"
            @click="loadPatientSummary">
        ...
        <!-- NAV ICONS -->
        <div v-for="icon in currentNavOptions" :key="icon.id" class="nav-card shadow-lg position-relative" role="button"
            tabindex="0" @click.stop="handleCardClick(icon)">
            <div class="nav-card-content" :data-tooltip="icon.label" data-tooltip-location="bottom">
                ...
                <i v-if="icon.label === 'Office' || icon.label === 'Configure'"
                    class="bi bi-caret-down-fill more-options"></i>
            </div>
        </div>

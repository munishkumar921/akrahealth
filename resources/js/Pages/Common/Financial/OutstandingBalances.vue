...
const columns = [
    ...
    { label: "Balance", key: "balance_label", type: "slot", slot: "balance", align: "left" },
    { label: "Notes", key: "billing_notes", type: "slot", slot: "notes", align: "left" },
];
...
const openPatientBilling = (row) => {
    router.get(route("doctor.billing.index", { type: "encounter", patient_id: row.patient_id }));
};
...
const resultSummary = computed(() => {
    ...
    return `Showing ${from}-${to} of ${total} patients with outstanding balances`;
});
</script>
...
<template>
    <AuthLayout2
        ...
        description="View patient outstanding balances"
                ...
                <div class="users-toolbar card border-0 shadow-sm mb-4">
                    <div class="card-body">
                        <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3 mb-3">
                            <div>
                                <h3 class="mb-1">Outstanding Balances</h3>
                        ...
                        <div class="row g-3 align-items-end">
                            <div class="col-12 col-sm-6 col-xl-4">
                                ...
                                <div class="input-group balances-search-control">
                                    ...
                                    <input
                                        ...
                                        placeholder="Search patient, notes, balance"
                    ...
                    <div class="card-body p-0 p-md-3">
                        <div class="d-flex justify-content-end align-items-center px-3 px-md-0 pt-3 pt-md-0 pb-2">
                            <div class="d-flex align-items-center gap-2 rows-select-wrap">
                                <label for="outstanding-balances-per-page" class="text-muted small text-uppercase mb-0">Rows</label>
                                <select
                                    id="outstanding-balances-per-page"
                                    v-model="perPage"
                        ...
                        <Table
                            ...
                            @cell-click="({ row }) => openPatientBilling(row)"
                        >
                            <template #patientName="{ row }">
                                <div class="text-start">
                                    <div class="fw-semibold text-dark">{{ row.patient_name || "-" }}</div>
                                    <div class="text-muted small">Patient ID: {{ row.patient_id || "-" }}</div>
                                </div>
                            </template>

                            <template #balance="{ row }">
                                <div class="text-start">
                                    <span class="balance-pill" :class="Number(row.balance || 0) > 0 ? 'balance-due' : 'balance-clear'">
                                        {{ row.balance_label || "-" }}
                                    </span>
                                </div>
                            </template>
                            ...
                            <template #notes="{ row }">
                                <div class="text-start">
                                    <div class="fw-medium text-dark">{{ row.billing_notes || "-" }}</div>
                                </div>
                            </template>
                        </Table>
                    </div>

...
const currentTab = ref("bills_to_submit");
...
const filterForm = ref({
    ...
    encounter_signed: props.filters?.encounter_signed || "",
});

const columns = [
    { label: "Date", key: "date_of_service_label", type: "slot", slot: "dateOfService", align: "left" },
    { label: "Patient Name", key: "patient_name", type: "slot", slot: "patientName", align: "left" },
    { label: "Status", key: "encounter_signed_label", type: "slot", slot: "status", align: "left" },
    { label: "Chief Complaint", key: "chief_complaint", type: "slot", slot: "chiefComplaint", align: "left" },
    { label: "Batch Type", key: "batch_type_label", type: "slot", slot: "batchType", align: "left" },
];
...
const resultSummary = computed(() => {
    ...
    return `Showing ${from}-${to} of ${total} pending bills`;
});
</script>

<template>
    <AuthLayout2 title="Bills to Submit" description="View bills pending submission" heading="Bills to Submit">
        <div class="row g-4">
                ...
                <div class="users-toolbar card border-0 shadow-sm mb-4">
                    <div class="card-body">
                        <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3 mb-3">
                            <div>
                                <h3 class="mb-1">Bills to Submit</h3>
                        ...
                        <div class="row g-3 align-items-end">
                            <div class="col-12 col-sm-6 col-xl-4">
                                ...
                                <div class="input-group bills-search-control">
                            ...
                            <div class="col-12 col-sm-6 col-xl-3">
                                ...
                                <select v-model="filterForm.encounter_signed" class="form-select" @change="applyFilters">
                    ...
                    <div class="card-body p-0 p-md-3">
                        <div class="d-flex justify-content-end align-items-center px-3 px-md-0 pt-3 pt-md-0 pb-2">
                            <div class="d-flex align-items-center gap-2 rows-select-wrap">
                                <label for="bills-to-submit-per-page" class="text-muted small text-uppercase mb-0">Rows</label>
                                <select
                                    id="bills-to-submit-per-page"
                                    v-model="perPage"
                        ...
                        <Table :columns="columns" :data="bills" :search-show="false" :PageOptions="false">
                            ...
                            <template #status="{ row }">
                                <div class="text-start">
                                    <span
                                        class="status-pill"
                                        :class="`status-${String(row.encounter_signed_label || 'pending').toLowerCase().replace(/\s+/g, '-')}`"
                                    >
                                        {{ row.encounter_signed_label || "-" }}
                                    </span>
                                </div>
                            </template>
                            ...
                            <template #batchType="{ row }">
                                <div class="text-start">
                                    <div class="fw-medium text-dark">{{ row.batch_type_label || "-" }}</div>
                                </div>
                            </template>
                        </Table>
                    </div>

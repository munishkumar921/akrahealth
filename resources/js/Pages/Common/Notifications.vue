...
const columns = [
  { label: 'Notification', key: 'message', type: 'slot', slot: 'message' },
...
const deleteNotification = async (notificationId) => {
  Swal.fire(confirmSettings("Are you sure you want to delete this notification?")).then(async (result) => {
              ...
              <div class="d-flex align-items-center">
                <div class="btn-group btn-group-sm" role="group">
                  <button type="button" class="btn"
                    ...
                    @click="fetchNotifications('all')">All</button>
                  <button type="button" class="btn"
                    :class="filterStatus === 'unread' ? 'btn-primary' : 'btn-outline-primary'"
                    @click="fetchNotifications('unread')">Unread</button>
                  <button type="button" class="btn"
                    :class="filterStatus === 'read' ? 'btn-primary' : 'btn-outline-primary'"
                    @click="fetchNotifications('read')">Read</button>
                </div>
                <button class="btn btn-sm btn-info ml-3" @click="markAllAsRead">Mark all as read</button>
              </div>
            </div>
            <div class="iq-card-body-bkp">
              <Table :columns="columns" :data="tableData" :searchShow="false"
                @cell-click="({ row }) => handleNotificationClick(row)">
                <template #message="{ row }">
                  <div :class="{ 'font-weight-bold': !row.is_read }">
                    <div>{{ row.title || 'Notification' }}</div>
                    <small class="text-muted">{{ row.message || 'No message available' }}</small>
                    <span v-if="!row.is_read" class="badge badge-primary ml-2">New</span>
                  </div>
                </template>
                <template #actions="{ row }">
                  <button class="btn btn-icon btn-danger" @click.stop="deleteNotification(row.id)"

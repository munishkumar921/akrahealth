...
const handleCheckboxChange = (item, option, isChecked) => {
    // Get all currently checked options for this checkbox group
    const checkboxes = document.querySelectorAll(`input[name="checkbox_${item.value}"]:checked`);
    const selectedValues = Array.from(checkboxes).map(cb => cb.value);

    // Emit the selected values
    emit('template-selected', {
        value: selectedValues.join(', '),
        text: `${props.category}: ${item.value}: ${selectedValues.join(', ')}`,
        category: props.category,
        inputType: item.input,
        options: item.options,
...
const handleRadioChange = (item, option) => {
    emit('template-selected', {
        value: option,
        text: `${props.category}:${item.value}: ${option}`,
        category: props.category,
        inputType: item.input,
        options: item.options,
...
const deleteTemplate = (template) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.append('group_name', selectedParentTemplate.value?.value ? selectedParentTemplate.value?.value : template?.value);
            ...
            <div class="card-body p-0 template-content">
                    ...
                    <div v-if="templates?.length > 0" class="categories-list">
                        <div v-for="template in templates" :key="template.value"
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <span class="template-name cursor-pointer" @click="getTemplateItems(template)">
                                    {{ template.value }}
                                </span>
                            </div>
                            <div class="template-actions">
                                <i @click="getTemplateItems(template)"
                                    class="fa fa-arrow-right cursor-pointer me-2 text-muted" title="View items"></i>
                                <i @click="openTemplateModal(template)"
                                    class="fa fa-pencil cursor-pointer me-2 text-primary" title="Edit"></i>
                                <i @click="deleteTemplate(template)" class="fa fa-trash cursor-pointer text-danger"
                                    title="Delete"></i>
                            </div>
                        </div>
                    </div>
                ...
                <div v-else class="template-items">
                    <div v-if="templateItems?.length > 0" class="items-list">
                        <div v-for="item in templateItems" :key="item.value" class="template-item border-bottom p-3">
                            <div class="d-flex justify-content-between align-items-center mb-2">
                                ...
                                <div class="template-item-actions">
                                    <i @click="openTemplateModal(item)"
                                        class="fa fa-pencil cursor-pointer me-2 text-primary" title="Edit"></i>
                                    <i @click="deleteTemplate(item)" class="fa fa-trash cursor-pointer text-danger"
                                        title="Delete"></i>
                                </div>
                            </div>
                            ...
                            <div v-if="['text', 'radio', 'checkbox', 'select'].includes(item.input)"
                                ...
                                <div v-if="item.input == 'text'" class="mb-2">
                                    <input type="text" class="form-control form-control-sm"
                                        ...
                                        @blur="handleTextChange(item, $event.target.value)" />
                                </div>
                                            ...
                                            <div class="col-6 mb-1">
                                                <label class="d-flex align-items-center small">
                                                    <input type="radio" class="me-2 cursor-pointer"
                                                        :name="'radio_' + item.value" :value="option.trim()"
                                                        @change="handleRadioChange(item, option.trim())" />
                                                    {{ option.trim() }}
                                                </label>
                                            </div>
                                        </template>
                                            ...
                                            <div class="col-6 mb-1">
                                                <label class="d-flex align-items-center small">
                                                    <input type="checkbox" class="me-2 cursor-pointer"
                                                        :name="'checkbox_' + item.value" :value="option.trim()"
                                                        @change="handleCheckboxChange(item, option.trim(), $event.target.checked)" />
                                                    {{ option.trim() }}
                                                </label>
                                            </div>
                                        </template>
                                ...
                                <div v-else-if="item.input == 'select'" class="mb-2">
                                    <select class="form-control form-control-sm"
                                        @change="handleSelectChange(item, $event.target.value)">
...
.template-container {
    height: 600px;
    /* Fixed height for the entire template container */
    display: flex;

...
const applyFilters = () => {
	router.get(route('admin.razorpay.transactions.list'), {
...
<template>
	...
	>
		<div class="d-flex align-items-center justify-content-between mb-4">
			<h3 class="d-flex align-items-center text-xl mb-2">Razorpay Transactions</h3>
		</div>
			...
			<div class="col-md-3">
				<div class="card bg-primary text-white">
					<div class="card-body">
						...
						<p class="card-text h3">{{ transactions.data?.length || 0 }}</p>
					</div>
				</div>
			</div>
			<div class="col-md-3">
				<div class="card bg-success text-white">
					<div class="card-body">
						...
						<p class="card-text h3">{{ transactions.data?.filter(t => t.status === 'completed' || t.status === 'active').length || 0 }}</p>
					</div>
				</div>
			</div>
			<div class="col-md-3">
				<div class="card bg-warning text-dark">
					<div class="card-body">
						...
						<p class="card-text h3">{{ transactions.data?.filter(t => t.status === 'pending').length || 0 }}</p>
					</div>
				</div>
			</div>
			<div class="col-md-3">
				<div class="card bg-danger text-white">
					<div class="card-body">
						...
						<p class="card-text h3">{{ transactions.data?.filter(t => t.status === 'failed').length || 0 }}</p>
					</div>
				</div>
			</div>
		</div>

...
const columns = [
    { label: "Speciality", key: "name", type: "slot", slot: "name", align: "left" },
...
const rows = computed(() => props.specialities?.data ?? []);
const activeFilterCount = computed(() =>
    Object.values(filterForm.value).filter((value) => value !== null && value !== "").length
);
const hasActiveFilters = computed(() => activeFilterCount.value > 0);
const resultSummary = computed(() => {
    const total = props.specialities?.total ?? rows.value.length;
    const from = props.specialities?.from ?? (rows.value.length ? 1 : 0);
    const to = props.specialities?.to ?? rows.value.length;

    if (!total) {
        return "No specialities found";
    }

    return `Showing ${from}-${to} of ${total} specialities`;
});
...
const applyFilters = () => {
    router.get(route("admin.specialities.index"), buildQuery(), {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const updatePerPage = () => {
    router.get(route("admin.specialities.index"), buildQuery({ per_page: perPage.value, page: 1 }), {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
        ...
        <div class="specialities-page">
                ...
                <div class="card-body">
                    ...
                    <div class="row g-3 align-items-end">
                        <div class="col-12 col-xl-4">
                            ...
                            <div class="input-group speciality-search-control">
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-body p-0 p-md-3">
                    ...
                    <Table :columns="columns" :data="specialities" :search-show="false" :PageOptions="false">
                        <template #name="{ row }">
                        ...
                        <template #actions="{ row }">
                            <div class="d-flex gap-2">
                                <button class="btn btn-light border" @click="openEdit(row)" title="Edit speciality">

...
const columns = [
    { label: "Category", key: "name", type: "slot", slot: "name", align: "left" },
...
const resultSummary = computed(() => {
    const total = props.categories?.total ?? rows.value.length;
    const from = props.categories?.from ?? (rows.value.length ? 1 : 0);
    const to = props.categories?.to ?? rows.value.length;

    if (!total) {
        return "No lab test categories found";
    }

    return `Showing ${from}-${to} of ${total} lab test categories`;
});
...
const applyFilters = () => {
    router.get(route("admin.lab-test-categories.index"), buildQuery(), {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const updatePerPage = () => {
    router.get(route("admin.lab-test-categories.index"), buildQuery({ per_page: perPage.value, page: 1 }), {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const removeRow = (row) => {
    Swal.fire(
        ...
        if (result.isConfirmed) {
            useForm({}).delete(route("admin.lab-test-categories.destroy", row.id));
        }
    });
};
...
<template>
    <AuthLayout title="Lab Test Categories" description="Manage lab test categories" heading="Lab Test Categories">
        <div class="lab-test-categories-page">
            <div class="users-toolbar card border-0 shadow-sm mb-4">
                <div class="card-body">
                    <div
                        ...
                        <div>
                            <h3 class="mb-1">Lab Test Categories</h3>
                    ...
                    <div class="row g-3 align-items-end">
                        <div class="col-12 col-xl-4">
                            ...
                            <div class="input-group category-search-control">
                ...
                <div class="card-body p-0 p-md-3">
                    <div class="d-flex justify-content-end align-items-center px-3 px-md-0 pt-3 pt-md-0 pb-2">
                        <div class="d-flex align-items-center gap-2 rows-select-wrap">
                            <select
                                id="lab-test-categories-per-page"
                                v-model="perPage"
                    ...
                    <Table :columns="columns" :data="categories" :search-show="false" :PageOptions="false">
                        <template #name="{ row }">
                        ...
                        <template #actions="{ row }">
                            <div class="d-flex gap-2">
                                <button class="btn btn-light border" @click="openEdit(row)" title="Edit category">
            ...
            <Modal :isOpen="isAddModalOpen" title="Lab Test Category Details" @close="closeAddModal" size="xl">

...
const perPage = ref(
    Number(new URLSearchParams(window.location.search).get("per_page")) || Number(props.ccrReports?.per_page) || 10
);
...
const resultSummary = computed(() => {
    const total = props.ccrReports?.total ?? props.ccrReports?.data?.length ?? 0;
    const from = props.ccrReports?.from ?? (total ? 1 : 0);
    const to = props.ccrReports?.to ?? total;

    if (!total) {
        return "No CCR reports found";
    }

    return `Showing ${from}-${to} of ${total} CCR reports`;
});

const visibleRows = computed(() => props.ccrReports?.data ?? []);
...
const applyFilters = () => {
    router.get(route("admin.CCRReports"), buildQuery({ page: 1 }), {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const clearFilters = () => {
    ...
    router.get(route("admin.CCRReports"), {}, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const updatePerPage = () => {
    router.get(route("admin.CCRReports"), buildQuery({ page: 1, per_page: perPage.value }), {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const downloadExcel = () => {
    if (!selectedRows.value.length) {
        toast("Please select at least one CCR report to export.");
        return;
    }
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "CCR Reports");
    XLSX.writeFile(workbook, "CCRReports.xlsx");
};
...
const openDocument = (row) => {
    ...
    if (!target) {
        toast("No document file available for this CCR report.");
        return;
    }
...
const downloadDocument = (row) => {
    ...
    link.href = target;
    link.download = row.file_name || "ccr-document";
...
<template>
    <AuthLayout title="CCR Reports" description="CCR Reports" heading="CCR Reports">
        <div class="ccr-reports-page">
            <div class="users-toolbar card border-0 shadow-sm mb-4">
                <div class="card-body">
                    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3 mb-3">
                        <div>
                            <h3 class="mb-1">CCR Reports</h3>
                    ...
                    <div class="row g-3 align-items-end">
                        <div class="col-12 col-xl-4">
                            ...
                            <div class="input-group ccr-search-control">

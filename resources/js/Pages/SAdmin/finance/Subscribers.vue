...
const resultSummary = computed(() => {
    const total = props.subscribers?.total ?? rows.value.length;
    const from = props.subscribers?.from ?? (rows.value.length ? 1 : 0);
    const to = props.subscribers?.to ?? rows.value.length;

    if (!total) return "No subscribers found";

    return `Showing ${from}-${to} of ${total} subscribers`;
});
...
const metricCards = computed(() => [
    {
        ...
        value: formatNumber(props.metrics?.total_subscribers ?? 0),
        helper: "All subscribers in the filtered result set",
    ...
    },
    {
        ...
        helper: "Subscribers currently active",
...
const applyFilters = () => {
    router.get(route("superAdmin.subscribers"), buildQuery(), {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const clearFilters = () => {
    ...
    router.get(route("superAdmin.subscribers"), {}, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const openSubscriber = (row) => {
    selectedSubscriber.value = row;
...
<template>
    <AuthLayout title="Subscribers" description="Platform subscription subscriber management">
        <section class="subscriber-page">
                ...
                <div class="card-body">
                    <div class="filter-header">
                        <div>
                            ...
                            <h3 class="filter-title">Refine subscribers</h3>
                        </div>
                        ...
                        <div class="col-12 col-xl-4">
                            ...
                            <div class="input-group">
                                ...
                                <input
                                    ...
                                    placeholder="Search user, email, plan, or subscription reference"
            ...
            <div class="card border-0 shadow-sm table-shell">
                <div class="card-body p-0 p-md-3">
                    <Table :columns="columns" :data="subscribers" table="superadmin-subscribers" :search-show="false">
                        <template #user="{ row }">
                            <div class="user-cell">
                        ...
                        <template #actions="{ row }">
                            <div class="d-flex justify-content-center">
                                <button class="icon-btn icon-btn--view" @click="openSubscriber(row)">
                                    <i class="bi bi-eye"></i>
                                </button>
                            </div>
                        </template>
        ...
        <Modal :isOpen="isDetailModalOpen" title="Subscriber Details" @close="closeModal" size="lg">
            <div v-if="selectedSubscriber" class="detail-grid">
                <div class="detail-item">
                    <label>User</label>
                    <span>{{ selectedSubscriber.user }}</span>
                </div>
                <div class="detail-item">
                    <label>Email</label>
                ...
                <div class="detail-item">
                    <label>Mobile</label>
                    <span>{{ selectedSubscriber.mobile }}</span>
                </div>
                <div class="detail-item">
                    <label>Plan</label>
                    <span>{{ selectedSubscriber.plan_name }}</span>
                </div>
                <div class="detail-item">
                    <label>Pricing</label>
                    <span>{{ selectedSubscriber.pricing_plan }}</span>
                </div>
                <div class="detail-item">
                    <label>Gateway</label>
                    <span>{{ selectedSubscriber.gateway_label }}</span>
                </div>
                <div class="detail-item">
                    <label>Status</label>
                ...
                <div class="detail-item">
                    <label>Payment</label>
                    <span>{{ selectedSubscriber.payment_status }}</span>
                </div>
                <div class="detail-item detail-item--wide">
                    <label>Subscription Reference</label>
                    <span>{{ selectedSubscriber.subscription_id }}</span>
                </div>
                <div class="detail-item">
                    <label>Subscribed On</label>
                    <span>{{ selectedSubscriber.subscribed_on }}</span>
                </div>
                <div class="detail-item">
                    <label>Next Payment</label>
                    <span>{{ selectedSubscriber.next_payment }}</span>
                </div>
                <div class="detail-item">
                    <label>Created</label>
                    <span>{{ selectedSubscriber.created_label }}</span>
                </div>
            </div>
        </Modal>
    </AuthLayout>

...
<head>
    ...
    <style>
        ...
        .letter-type { text-align: center; font-size: 16pt; font-weight: bold; color: #3498db; margin-bottom: 30px; text-transform: uppercase; letter-spacing: 2px; }
        .date { text-align: right; margin-bottom: 20px; }
        .patient-info { background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin-bottom: 20px; border-left: 4px solid #3498db; }
        ...
        .findings-box { background-color: #e8f4fc; padding: 20px; border-radius: 5px; margin: 20px 0; }
        .recommendations-box { background-color: #e8f8f5; padding: 20px; border-radius: 5px; margin: 20px 0; border-left: 4px solid #27ae60; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <div style="text-align: center;"><div class="letter-type">CONSULTATION REPORT</div></div>
    ...
    <div class="findings-box"><strong style="color: #3498db;">CLINICAL FINDINGS:</strong><br>{{ $body }}</div>
    <div class="recommendations-box"><strong style="color: #27ae60;">RECOMMENDATIONS:</strong><br>Please see detailed recommendations in the body of this report.</div>
    <div class="signature-section"><div class="signature-line"></div><div class="doctor-info"><strong>Dr. {{ $doctor['name'] ?? 'N/A' }}</strong><br>{{ $doctor['specialty'] ?? 'General Practice' }}<br>@if(($doctor['qualification'] ?? '')){{ $doctor['qualification'] }}<br>@endif @if(($doctor['registration_number'] ?? ''))Reg. No: {{ $doctor['registration_number'] }}<br>@endif</div></div>
    ...
    <div class="footer"><div style="text-align: center; font-size: 9pt; color: #999;">This consultation report is confidential and intended for the named recipient.</div></div>
</body>
</html>
